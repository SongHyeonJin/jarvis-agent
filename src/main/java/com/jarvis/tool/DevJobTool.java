...
public class DevJobTool implements ToolProvider {
    ...
    public List<ToolFunction> getToolFunctions() {
        return List.of(recentJobs(), currentGitStatus());
    }

    private ToolFunction recentJobs() {
        return new ToolFunction() {
            @Override public String name() { return "getRecentDevJobs"; }

            @Override public String description() {
                return "최근 개발 작업(DevJob) 이력을 조회합니다. " +
            ...
            @Override public Map<String, Object> parameters() {
                return Map.of(
                    ...
                    "properties", Map.of(
                        "limit", Map.of(
                            "type", "integer",
                            "description", "조회할 작업 수 (기본값 5, 최대 20)"
                        )
                    )
                );
            }
            ...
            @Override public String execute(Map<String, Object> args) {
                int limit = args.get("limit") != null
                    ? Math.min(((Number) args.get("limit")).intValue(), 20)
                    : 5;
    ...
    private ToolFunction currentGitStatus() {
        return new ToolFunction() {
            @Override public String name() { return "getCurrentGitStatus"; }

            @Override public String description() {
                return "현재 git 저장소의 변경 상태(status, diff --stat, 브랜치)를 조회합니다. " +
